...
public class StorageItemPreferenceController extends AbstractPreferenceController implements
    ...
    private void updatePrivateStorageCategoryPreferencesOrder() {
        ...
        for (StorageItemPreference preference : mPrivateStorageItemPreferences) {
            ...
            applyCardLayout(orderIndex, preference);
        }
    }

    private void applyCardLayout(int CurrentOrder, Preference pref) {
        if (pref == null ) {
            Log.e(TAG, "preference is null!");
            return;
        }
        String key = pref.getKey();
        //Log.i(TAG, "Setting layout for preference :- " + key + ", Current Order :- " + CurrentOrder);
        pref.setLayoutResource(org.elixir.resources.cardlayout.R.layout.storage_item_mid);
        if (CurrentOrder == 199) {
        //    Log.i(TAG, "Setting Top layout for preference :- " + key + ", Current Order :- " + CurrentOrder);
            pref.setLayoutResource(org.elixir.resources.cardlayout.R.layout.storage_item_bot);
        }
        if (CurrentOrder == 192) {
        //    Log.i(TAG, "Setting Bottom layout for preference :- " + key + ", Current Order :- " + CurrentOrder);
            pref.setLayoutResource(org.elixir.resources.cardlayout.R.layout.storage_item_top);
        }
    }
    ...
    public void onLoadFinished(@Nullable SparseArray<StorageAsyncLoader.StorageResult> result,
        ...
        if (mSystemPreference != null) {
            ...
            mSystemPreference.setLayoutResource(org.elixir.resources.cardlayout.R.layout.xd_pref_card_top);
        }
        if (mTemporaryFilesPreference != null) {
            mTemporaryFilesPreference.setStorageSize(storageCache.temporaryFilesSize, mTotalSize,
                    animate);
            mTemporaryFilesPreference.setLayoutResource(org.elixir.resources.cardlayout.R.layout.xd_pref_card_bot);
        }
        // Cache the size info
        if (result != null) {
